        ...
        <div>
            ...
            <div className="mb-6 flex items-center space-x-2 overflow-x-auto pb-2">
                {(['All', 'Exam', 'Event', 'General', 'Scholarship', 'Club'] as const).map(cat => (
                    <button key={cat} onClick={() => setFilter(cat)} className={`flex-shrink-0 px-4 py-1.5 text-sm font-semibold rounded-full transition-all ${filter === cat ? 'bg-primary-500 text-white shadow-md' : 'bg-slate-700 text-slate-300 hover:bg-slate-600'}`}>{cat}</button>
                ))}
            </div>

            {filteredNotices.length === 0 ? (
                <div className="text-center py-20 bg-slate-800/80 rounded-lg border border-dashed border-slate-700">
                    <h2 className="text-2xl font-semibold text-slate-300">No Notices Found</h2>
                    <p className="text-slate-500 mt-2">The data set is not currently available and will be added soon.</p>
                    {user?.isAdmin && <button onClick={() => openModal()} className="mt-4 px-4 py-2 bg-primary-600 hover:bg-primary-700 rounded-lg text-white font-semibold transition-colors">Add First Notice</button>}
                </div>
            ) : (
